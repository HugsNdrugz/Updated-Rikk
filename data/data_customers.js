export const customerArchetypes = {
    ...
    HIGH_ROLLER: {
        ...
        dialogueVariations: {
            ...
            rikkPriceTooHigh: (mood) => { 
                ...
                return [line, `Rikk, please. **That price is an insult to both my intelligence and my tailor.** I have a counter-proposal, if you're wise enough to hear it.`]; 
            },
            rikkBuysSuccess: (mood) => {
                const lines = {
    ...
    STIMULANT_USER: {
        ...
        sellPreference: (item) => (item.itemTypeObj.type === "STOLEN_GOOD" && item.qualityIndex === 0) || item.id === "half_baked_invention_idea" || item.id === "blueprint_for_squirrel_armor", 
    ...
    PSYCHEDELIC_EXPLORER: {
        ...
        sellPreference: (item) => item.id === "perfectly_normal_rock_portal_key" || item.id === "sentient_dust_bunny_wisdom" || (item.itemTypeObj.type === "STOLEN_GOOD" && item.qualityIndex === 0 && Math.random() < 0.3), 
