// =================================================================================
// Game Data: Customer Archetypes
// =================================================================================
// This file is the definitive source of truth for all customer personalities,
// dialogue, and behavior in the game. It is designed to be a self-contained ES6
// module that exports a single `customerArchetypes` object.
//
// Each archetype is structured to be independent of the main game script's global
// state. Functions like `postDealEffect` return objects describing the desired
// changes, which the main script is responsible for applying.
// =================================================================================

export const customerArchetypes = {
    // =============================================================================
    // ARCHETYPE 1: DESPERATE FIEND
    // =============================================================================
    DESPERATE_FIEND: {
        key: "DESPERATE_FIEND",
        ...
        dialogueVariations: {
            lowCashRikk: (mood) => {
                ...
                return lines[mood] || lines.default;
            },
            ...
            rikkSellsSuccess: (mood) => {
             ...
             negotiationSuccess: (mood) => ["Aight, aight, fine. Deal. Just hurry it up."],
             negotiationFail: (mood) => ["Nah man, nah. My first offer stands. That's it."],
        },
        postDealEffect: (success, customerState) => {
            if (success && Math.random() < 0.15 && customerState.mood !== "happy") {
                 return {
                     heatChange: 3,
                     message: `${customerState.name} stumbles away looking REALLY rough... Hope they're okay. Or not your problem. (+3 Heat)`
                 };
            }
            return null;
        },
    ...
    HIGH_ROLLER: {
        ...
        greeting: (customer, item) => {
            ...
            const moodKey = customer.mood === "paranoid" ? "paranoid" : customer.mood === "happy" ? "happy" : "arrogant";
            return greetings[moodKey][Math.floor(Math.random() * greetings[moodKey].length)];
        },
        buyPreference: (item) => (item.itemTypeObj.type === "DRUG" && item.qualityIndex === 2) || (item.itemTypeObj.type === "STOLEN_GOOD" && item.qualityIndex >= 1 && item.itemTypeObj.baseValue > 100),
        ...
        dialogueVariations: {
            lowCashRikk: (mood) => ["Penniless, Rikk? How... droll. An amusing anecdote for my club, perhaps. Do try to be solvent for our next encounter.", "A liquidity problem? Unfortunate. I require partners who can keep pace."],
            rikkDeclinesToBuy: (mood) => ["You're passing on this? An interesting, if foolish, business decision. Your loss.", "I see. You lack the... vision to appreciate true quality. Pity."],
            rikkDeclinesToSell: (mood) => ["You refuse my generous offer? Very well. There are other... vendors... with fewer reservations.", "Disappointing. I had hoped you were more astute. I will procure my necessities elsewhere."],
            rikkBuysSuccess: (mood) => ["A prudent choice. The funds have been transferred.", "As expected. Your service is noted, Rikk. Continue to provide this level of quality."],
            rikkSellsSuccess: (mood) => ["Excellent. This will suffice. I expect this level of quality to be maintained.", "Marvelous! This will pair exquisitely with my evening's... *endeavors*."],
            negotiationSuccess: (mood) => ["Fine. Let's be expedient. Your price is... acceptable.", "Very well, you've made your point. We have a deal."],
            negotiationFail: (mood) => ["I don't haggle, Rikk. My offer is my offer. Take it or I shall take my business elsewhere.", "Let's not be vulgar, Rikk. That price is an insult. My offer stands. It is final."],
        },
        postDealEffect: (success, customerState, currentCash) => {
            if (success && Math.random() < 0.1) {
                const tip = Math.floor(currentCash * 0.05);
                return {
                    cashChange: tip,
                    credChange: 1,
                    message: `${customerState.name} was exceptionally pleased and tipped you $${tip}! (+1 Cred)`
                };
            }
            return null;
        },
    ...
    REGULAR_JOE: {
        ...
        sellPreference: (item) => item.qualityIndex <= 1 && item.itemTypeObj.type === "STOLEN_GOOD" && item.itemTypeObj.baseValue < 100,
        priceToleranceFactor: 0.9,
        negotiationResists: false,
        heatImpact: 1, credImpactSell: 1, credImpactBuy: 0,
        initialMood: "chill",
        dialogueVariations: {
            lowCashRikk: (mood) => ["Ah, bummer dude. No worries, hit me up when the wallet's fatter.", "Damn, all good man. I'll circle back."],
            rikkDeclinesToBuy: (mood) => ["No sweat, man. Worth a shot. Catch you later.", "Aight, no biggie."],
            rikkDeclinesToSell: (mood) => ["For real? Aight, man. Your call. Peace.", "All good, maybe next time."],
            rikkBuysSuccess: (mood) => ["Cool, appreciate it. Keeps the dream alive, or at least the Wi-Fi on.", "Right on. Good deal. Later, Rikk."],
            rikkSellsSuccess: (mood) => ["Nice one, Rikk. Just what the doctor didn't order, but what my soul needed.", "Perfect. Time to kick back and let the good times roll."],
            negotiationSuccess: (mood) => ["Yeah, that's solid. Good looking out.", "Cool, cool. That works for me. Appreciate it, my dude."],
            negotiationFail: (mood) => ["Ah, that's a little rich for my blood, Rikk. Gotta watch the budget, you know? Adulting and all that jazz.", "Nah, that's a bit much for me today, bro. Maybe another time."],
        },
        postDealEffect: null,
    },

    // =============================================================================
    // ARCHETYPE 4: INFORMANT
    // =============================================================================
    INFORMANT: {
        key: "INFORMANT",
        ...
        dialogueVariations: {
            rikkCannotAfford: (mood) => ["This ain't a charity, Rikk. My whispers have value. You want the dirt, you gotta pay for the shovel.", "Look, Rikk. Good intel costs. Bad intel costs more. Your call."],
            rikkDeclinesToBuy: (mood) => ["Suit yourself. The streets are talking. Just a matter of who's listening.", "A mistake, but it's yours to make. Information this fresh doesn't stay on the shelf long."],
            rikkBuysSuccess: (mood) => ["Solid. Use that wisely, it could save your hide. Or make you a mint. Keep my number.", "Good. Remember where you got it. And remember, some doors are best left unopened... unless you have a key. Which I just sold you."],
        },
        postDealEffect: null,
    },
    ...
    SNITCH: {
        ...
        initialMood: "nosy",
        dialogueVariations: {
            lowCashRikk: (mood) => ["Oh, you can't afford it? That's... interesting. Very interesting indeed. I'll make a note of that."],
            rikkDeclinesToSell: (mood) => ["Oh, really? Well, alright then. Just trying to be friendly! One never knows what interesting things are about!", "Keeping it to yourself, Rikk? Mysterious! I like a good mystery."],
            rikkSellsSuccess: (mood) => ["Splendid! Thank you, Rikk! This is just perfect for my... collection. You're such a vital part of the... local color!", "Oh, wonderful! This will make an excellent... *exhibit* in my report... I mean, my scrapbook!"],
        },
        postDealEffect: (success, customerState) => {
            if (success && Math.random() < 0.65) {
                const snitchHeat = Math.floor(Math.random() * 15) + 10;
                return {
                    heatChange: snitchHeat,
                    credChange: -2,
                    message: `🚨 RAT ALERT! 🚨 **${customerState.name}** was seen yapping to the 5-0! (+${snitchHeat} Heat, -2 Cred)`
                };
            } else if (success) {
                 return { message: `You feel **${customerState.name}'s** beady eyes on you as they leave... **like a human CCTV camera.**` };
            }
            return null;
        }
    },
